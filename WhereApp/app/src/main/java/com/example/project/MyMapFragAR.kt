...
class MyMapFragAR(var arCamera: ARCamera, var googleMap:GoogleMap) {
    ...
    init{
        ...
        */
      //  places.add(MyPlaceAR(arCamera,"학원뒤",37.50044475543722 , 127.02550080356122,Color.GREEN))
        Log.d("mapPinAR ","${mapPinAR.longitude}")
        Log.d("mapPinAR ","${mapPinAR.latitude}")
        places = mapPinAR!!.makePlaces(arCamera)


        earth = arCamera.mSession!!.earth!!

        arCamera.runOnUiThread{
            //지도에 띄우기

                googleMap.addMarker(MarkerOptions().position(places.latLng).title(places.title))
...
//        uiSettings.isTiltGesturesEnabled = false
    ...
    fun placeGLSurfaceMarking(){
        ...
        var scale = 50f
            ...
            val matrix = earthToMatrix(places.latLng)
            Matrix.scaleM(matrix,0,scale,scale,scale)
            places.obj.setModelMatrix(matrix)
